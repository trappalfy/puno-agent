 ...
 * sell.
 *
 * Dollar amounts are shown rather than PUNO amounts even though payment is in
 * PUNO: the token price is a per-request server lookup this static marketing
 * page has no session for, and a stale rate quoted on a landing page is worse
 * than an honest "priced in dollars, paid in PUNO".
 */
    ...
    <section id="pricing" className="relative z-10 overflow-hidden px-6 py-16 md:py-24">
      ...
      <div className="relative z-[3] mt-14 -mx-6 px-6 flex gap-4 overflow-x-auto no-scrollbar snap-x snap-mandatory lg:mx-auto lg:max-w-5xl lg:grid lg:grid-cols-3 lg:gap-5 lg:overflow-visible lg:px-0">
        {ROWS.map((row, i) => (
          ...
          >
            ...
            <div className="mt-1.5 text-4xl font-semibold tracking-tight text-lime-phosphor font-jetbrains-mono tabular-nums">
              ${PRICES_USD[row.key].toFixed(2)}
            </div>
      ...
      <div className="relative z-[3] mt-10 flex flex-col items-center gap-4">
        ...
        <p className="text-xs text-white/40">
          Priced in dollars, paid in PUNO. Bring your own Anthropic key and thinking is free.
        </p>
      </div>
    </section>
  );
