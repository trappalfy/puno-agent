...
| **B4**  | **Equity oracles stop outside market hours.** Measured 2026-08-15 (a Saturday): all five equity feeds 25–30 h stale, last published Friday inside the US session, while USDG/ETH/BTC were ~4 h. Against `EQUITY_STALENESS = 1 h` the vault's `_nav()` reverts, so the agent is mute nights and weekends — roughly 81% of wall-clock time — while the Uniswap pools keep quoting the whole time. **Needs a product decision, not a code fix.** Full write-up and three options in `EQUITY-FEED-HOURS-2026-08-15.md`. | `contracts/`, `apps/agent/src/loop/`, product copy                                  | **Open — decision owed**    |
