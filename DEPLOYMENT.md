...
Если сборка всё же упадёт на `Cannot find module '@puno/shared'`, значит это рассуждение неверно и
проверкой оказался сам деплой. Тогда обходной путь — оставить Root Directory пустым (корень
репозитория) и задать в дашборде Output Directory `apps/web/.next`; команды из `vercel.json` уже
написаны так, что работают из корня. Впишите сюда, что именно сработало.
