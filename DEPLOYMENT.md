...
5. **Environment Variables** — заполните **до** первого Deploy, три штуки, для Production и
   Preview:
...
**Пропустить шаг 5 и добавить переменные потом — значит гарантированно уронить первую сборку**,
и сообщение не укажет на причину. Проверено 2026-08-16. `apps/web/src/lib/db.ts` бросает
исключение прямо при импорте модуля, если `DATABASE_URL` пуст, а фаза `Collecting page data`
импортирует все тринадцать route-модулей, чтобы определить их статичность. В логе это выглядит
как `Failed to collect page data for /api/agents/[id]` — маршрут, который просто оказался первым,
и о котором нет ничего особенного. Лечится добавлением переменных и **Redeploy**: они
применяются со следующей сборки, поэтому повторный запуск той же ничего не даст.

Обратная сторона того же факта: сборке нужна боевая строка подключения, а `Preview` по умолчанию
получает ту же самую. Значит превью-деплои ходят в продакшн-базу. Пока пользователей нет, это
безразлично; перед тем как они появятся, Preview нужна отдельная ветка Neon.
